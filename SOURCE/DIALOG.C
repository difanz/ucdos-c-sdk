...
void UC_DrawAbout(WINDOWS *wnd)
{
   ...
   UC_GetFreeXMS(&tx, &fx);
   fc=coreleft();
   color_depth=UC_GetColourDepth();
   switch (color_depth) {
   case 3: strcpy(COLORSTATE, "单"); break;
   case 1: strcpy(COLORSTATE, "16"); break;
   case 2: strcpy(COLORSTATE, "256"); break;
   case 12: strcpy(COLORSTATE, "32K"); break;
   case 13: strcpy(COLORSTATE, "64K"); break;
   case 14: strcpy(COLORSTATE, "16.8M"); break;
   case 15: strcpy(COLORSTATE, "16.8M+A"); break;
   default: strcpy(COLORSTATE, "???"); break;
   }
   setcolor(0);
   setcolorbm(15);
   settextstyle(0, 0, wnd->syschar_size);
   UC_WindowPrintf(wnd, 60, 170, DT_NOOVER,
                   "可用常规内存 : %lu 字节\n总共扩展内存 : %i KB\n可用扩展内存 : %i KB\n当前屏幕设定 : %i×%i-%s色",
                   fc, tx, fx, getmaxx(), getmaxy()+20, COLORSTATE);
   UC_MouseShow();
}

void AboutMouse(int x, int y, WORD s)
{
   x=x;
   y=y;
