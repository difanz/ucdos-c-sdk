...
char *UC_GetNextFileNameByCount(int attr)
{
    DOS_CHECKDISK = 1;
    DOS_CHECKRETERR = -1;
...
char *UC_GetFileNameByCount(const char *spec, int count, int attr)
{
    int i = 0;
    DOS_CHECKDISK = 1;
    DOS_CHECKRETERR = -1;
    if (findfirst(spec, &ffblk, 0x3f) != 0)
        return NULL;
    do {
        if (attr & FA_DIREC) {
            if (ffblk.ff_attrib & FA_DIREC) {
...
int UC_GetFileTotal(const char *spec, int attr)
{
    int count = 1;
    if (UC_GetFileNameByCount(spec, count, attr) == NULL)
        return 0;
    while (UC_GetNextFileNameByCount(attr) != NULL)
        count++;
    return count;
}
...
int UC_GetDiskAttribute(int drv)
{
    unsigned char val;
    int cur_drv;
    ...
    if (drv <= 1) {
        ...
        if (cur_drv != 0) {
            if ((int)val <= drv && (int)val + cur_drv > drv)
                return 0x11;
        }
        return 0x10;
    }
